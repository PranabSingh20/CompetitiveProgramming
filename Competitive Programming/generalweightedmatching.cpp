//https://judge.yosupo.jp/submission/53112
...
void solve() {
    ...
    cin >> n >> m;
    vector<Edge> v(m);
    for (auto& [u, v, cost] : v) cin >> u >> v >> cost, ++u, ++v;
    MWM match(n, v);
    ll w = match.maximum_weighted_matching();
    auto ans = match.get_matching_edges();
    cout << ans.size() << ' ' << w << '\n';
    for (auto [x, y, w] : ans) cout << --x << ' ' << --y << '\n';
}